...
flowchart TD
    ...
    D --> E[Сбор всех токенов<br>с их позициями]
    E --> F[Сортировка токенов<br>по позиции в строке]
    F --> G{Есть переменная X?}
    G -- Да --> H[Запрос значения X<br>у пользователя]
    ...
    subgraph J [Предобработка токенов]
        ...
        K --> L[Замена "-" на "~"<br>для унарных операций]
    end
    ...
    subgraph M [Алгоритм преобразования в ОПЗ]
        direction TB
        M1[Инициализация<br>выход = вектор, стек = пустой] --> M2[Цикл по всем токенам]
        ...
        M3 -- Функция sin/cos/tg/ctg/exp --> M6[Поместить в стек операторов]
        ...
        subgraph M8 [Обработка ")"]
            M9[Пока верх стека ≠ "("] --> M10[Переместить оператор<br>из стека в выход]
        end
        
        subgraph M11 [Обработка бинарного оператора]
            M12[Пока стек не пуст И<br>верх стека ≠ "(" И<br>приоритет верха ≥ приоритету текущего] --> M13[Переместить оператор<br>из стека в выход]
            M13 --> M12
        ...
        subgraph M16 [Завершение преобразования]
            M17[Пока стек не пуст] --> M18[Переместить оператор<br>из стека в выход]
            M18 --> M17
        end
    end
    
    M --> N[Получена ОПЗ<br>постфиксная запись]
    N --> O
    ...
    subgraph O [Вычисление ОПЗ]
        ...
        O3 -- Число --> O4[Преобразовать в double,<br>поместить в стек]
        O3 -- Константа PI/E --> O5[Взять значение константы,<br>поместить в стек]
        O3 -- Переменная X --> O6[Взять значение X,<br>поместить в стек]
        O3 -- Унарный минус "~" --> O7[Извлечь a из стека,<br>вычислить -a, поместить результат]
        O3 -- Функция --> O8[Извлечь a из стека,<br>вычислить f(a), поместить результат]
        O3 -- Бинарный оператор --> O9[Извлечь b, затем a из стека,<br>вычислить a op b, поместить результат]
    ...
    O --> P[Вывод результата<br>на экран]
    P --> Q[Конец программы]
```
